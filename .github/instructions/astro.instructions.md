...
- Define a `Props` interface in frontmatter for every reusable component and document the interface with a short TSDoc comment that explains the component contract.
- Document each non-obvious prop in the `Props` interface with an inline TSDoc comment, including accepted values and nullability where relevant.
...
- Comment intent, rationale, and constraints — explain **why** a component or code path exists.
- Do not add comments that merely paraphrase markup, props, or expressions the code already makes clear.
- Keep comments next to the API or decision they describe, and update or delete them whenever the related code changes.
- Use a short TSDoc comment for every reusable component's `Props` interface so its public contract is discoverable.
